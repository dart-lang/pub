...
Generated by pub on ${DateTime.now()}.''';
  ...
  ///
  /// Returns a list of the packages where the contentHash from the cache is
  /// inserted into the description.
  List<PackageId> checkContentHashes(SystemCache cache) {
    return packages.values.map((id) {
      ...
      if (description is ResolvedHostedDescription) {
        ...
        if (cachedHash == null) {
          ...
          log.fine(
              'No hash of downloaded archive for ${id.name}-${id.version} found');
          ...
          if (!bytesEquals(cachedHash, description.sha256)) {
            // TODO(sigurdm): The error message would read better if the hash
            // was compared to the hash from the package-listing.
            dataError('''
Cache entry for ${id.name}-${id.version} does not have content-hash matching pubspec.lock.

This might indicate

* A corruption of your cache (try running `$topLevelProgram pub cache repair`).
* A corruption of the package repository (the package archive has been changed
  on the server).
...
''');
  ...
  String serialize(String? packageDir, SystemCache cache) {
    final packages = checkContentHashes(cache);
