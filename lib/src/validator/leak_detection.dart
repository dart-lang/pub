...
class LeakDetectionValidator extends Validator {
  ...
  Future<void> validate() async {
    // Load `false_leaks` from `pubspec.yaml`.
    final falseLeaks = Ignore(
      entrypoint.root.pubspec.falseLeaks,
    ...
    final leaks = await Future.wait(entrypoint.root.listFiles().map((f) async {
      ...
      // Skip files matching patterns in `false_leaks`
      final nixPath = p.posix.joinAll(p.split(relPath));
      if (falseLeaks.ignores(nixPath)) {
        return <LeakMatch>[];
      }
