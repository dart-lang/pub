...
/// out-of-date. It's expected to regenerate a snapshot at [snapshotPath], after
/// which the snapshot will be re-run. It's ignored if [snapshotPath] isn't
/// passed.
///
...
Future<int> runExecutable(Entrypoint entrypoint, String package,
  ...
  if (!isDart2 && fileExists(snapshotPath)) {
    // Since we don't access the package graph, this doesn't happen
    // automatically.
    entrypoint.assertUpToDate();

    return await runSnapshot(snapshotPath, args,
  ...
  var executablePath =
      await _executablePath(entrypoint, packagesFile, package, executable);
...
Future<String> _executablePath(Entrypoint entrypoint, String packagesFile,
    String package, String path) async {
  assert(p.isRelative(path));

  if (!fileExists(packagesFile)) {
    if (!entrypoint.isGlobal) return null;
    // A .packages file may not already exist if the global executable has a
    // 1.6-style lock file instead.
    await writeTextFile(packagesFile, entrypoint.packagesFileContents);
  }
