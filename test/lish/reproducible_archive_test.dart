...
void main() {
  test(
    'creates byte-for-byte identical archives with --reproducible regardless '
    'of file mtime or creation order',
    () async {
      ...
      await runPub(
        args: ['publish', '--to-archive=../archive1.tar.gz', '--reproducible'],
      );
      ...
      await runPub(
        args: ['publish', '--to-archive=../archive2.tar.gz', '--reproducible'],
      );
  ...
  test('cannot use both --from-archive and --reproducible', () async {
    await d.validPackage().create();

    await runPub(
      args: ['publish', '--from-archive=../archive.tar.gz', '--reproducible'],
      error: contains('Cannot use both --from-archive and --reproducible.'),
