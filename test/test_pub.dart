...
/// enabled.
Matcher isMinifiedDart2JSOutput = isNot(contains('// The code supports the following hooks'));

/// A [Matcher] that matches JavaScript generated by dart2js with minification
/// disabled.
Matcher isUnminifiedDart2JSOutput = contains('// The code supports the following hooks');
...
String packageSpecLine(String packageName) => File(d.path(packagesFilePath)).readAsLinesSync().firstWhere((l) => l.startsWith('$packageName:'));
...
/// output.
class RunCommand {
  static final add = RunCommand('add', RegExp(r'Got dependencies!|Changed \d+ dependenc(y|ies)!'));
  static final get = RunCommand('get', RegExp(r'Got dependencies!|Changed \d+ dependenc(y|ies)!'));
...
Try `pub outdated` for more information.$)'''));
  static final downgrade = RunCommand('downgrade', RegExp(r'(No dependencies changed\.|Changed \d+ dependenc(y|ies)!)$'));
  static final remove = RunCommand('remove', RegExp(r'Got dependencies!|Changed \d+ dependenc(y|ies)!'));
...
Future pubCommand(RunCommand command, {Iterable<String> args, output, error, silent, warning, int exitCode, Map<String, String> environment}) async {
  ...
  await runPub(args: allArgs, output: output, error: error, silent: silent, exitCode: exitCode, environment: environment);
}

Future pubAdd({Iterable<String> args, output, error, warning, int exitCode, Map<String, String> environment}) => pubCommand(RunCommand.add, args: args, output: output, error: error, warning: warning, exitCode: exitCode, environment: environment);

Future pubGet({Iterable<String> args, output, error, warning, int exitCode, Map<String, String> environment}) => pubCommand(RunCommand.get, args: args, output: output, error: error, warning: warning, exitCode: exitCode, environment: environment);

Future pubUpgrade({Iterable<String> args, output, error, warning, int exitCode, Map<String, String> environment}) => pubCommand(RunCommand.upgrade, args: args, output: output, error: error, warning: warning, exitCode: exitCode, environment: environment);

Future pubDowngrade({Iterable<String> args, output, error, warning, int exitCode, Map<String, String> environment}) => pubCommand(RunCommand.downgrade, args: args, output: output, error: error, warning: warning, exitCode: exitCode, environment: environment);

Future pubRemove({Iterable<String> args, output, error, warning, int exitCode, Map<String, String> environment}) => pubCommand(RunCommand.remove, args: args, output: output, error: error, warning: warning, exitCode: exitCode, environment: environment);
...
Future<PubProcess> pubRun({bool global = false, Iterable<String> args, Map<String, String> environment, bool verbose = true}) async {
...
Future runPub({List<String> args, output, error, outputJson, silent, int exitCode = exit_codes.SUCCESS, String workingDirectory, Map<String, String> environment}) async {
  // Cannot pass both output and outputJson.
  assert(output == null || outputJson == null);

  var pub = await startPub(args: args, workingDirectory: workingDirectory, environment: environment);
...
Future<PubProcess> startPublish(PackageServer server, {List<String> args}) async {
...
Future confirmPublish(TestProcess pub) async {
  ...
  await expectLater(pub.stdout, emits(startsWith('Publishing test_pkg 1.0.0 to ')));
  await expectLater(
      pub.stdout,
...
/// If [environment] is given, any keys in it will override the environment
/// variables passed to the spawned process.
Future<PubProcess> startPub({Iterable<String> args, String tokenEndpoint, String workingDirectory, Map<String, String> environment, bool verbose = true}) async {
  args ??= [];
  ...
  return await PubProcess.start(dartBin, dartArgs, environment: getPubTestEnvironment(tokenEndpoint)..addAll(environment ?? {}), workingDirectory: workingDirectory ?? _pathInSandbox(appPath), description: args.isEmpty ? 'pub' : 'pub ${args.first}');
}
...
class PubProcess extends TestProcess {
  StreamSplitter<Pair<log.Level, String>> get _logSplitter {
    __logSplitter ??= StreamSplitter(StreamGroup.merge([_outputToLog(super.stdoutStream(), log.Level.MESSAGE), _outputToLog(super.stderrStream(), log.Level.ERROR)]));
  ...
  static Future<PubProcess> start(String executable, Iterable<String> arguments,
      {String workingDirectory, Map<String, String> environment, bool includeParentEnvironment = true, bool runInShell = false, String description, Encoding encoding, bool forwardStdio = false}) async {
    var process = await Process.start(executable, arguments.toList(), workingDirectory: workingDirectory, environment: environment, includeParentEnvironment: includeParentEnvironment, runInShell: runInShell);

    if (description == null) {
      var humanExecutable = p.isWithin(p.current, executable) ? p.relative(executable) : executable;
      description = '$humanExecutable ${arguments.join(' ')}';
    }

    encoding ??= utf8;
    return PubProcess(process, description, encoding: encoding, forwardStdio: forwardStdio);
  }

  /// This is protected.
  PubProcess(process, description, {Encoding encoding, bool forwardStdio = false}) : super(process, description, encoding: encoding, forwardStdio: forwardStdio);

  final _logLineRegExp = RegExp(r'^([A-Z ]{4})[:|] (.*)$');
  final Map<String, log.Level> _logLevels = [log.Level.ERROR, log.Level.WARNING, log.Level.MESSAGE, log.Level.IO, log.Level.SOLVER, log.Level.FINE].fold({}, (levels, level) {
    levels[level.name] = level;
    return levels;
  });

  Stream<Pair<log.Level, String>> _outputToLog(Stream<String> stream, log.Level defaultLevel) {
    log.Level lastLevel;
...
/// hosted packages.
Future createLockFile(String package, {Iterable<String> sandbox, Map<String, String> hosted}) async {
  var cache = SystemCache(rootDir: _pathInSandbox(cachePath));

  var lockFile = _createLockFile(cache.sources, sandbox: sandbox, hosted: hosted);

  await d.dir(package, [d.file('pubspec.lock', lockFile.serialize(null)), d.file('.packages', lockFile.packagesFile(cache, package))]).create();
}

/// Like [createLockFile], but creates only a `.packages` file without a
/// lockfile.
Future createPackagesFile(String package, {Iterable<String> sandbox, Map<String, String> hosted}) async {
  var cache = SystemCache(rootDir: _pathInSandbox(cachePath));
  var lockFile = _createLockFile(cache.sources, sandbox: sandbox, hosted: hosted);

  await d.dir(package, [d.file('.packages', lockFile.packagesFile(cache, package))]).create();
}
...
/// [hosted] is a list of package names to version strings for dependencies on
/// hosted packages.
LockFile _createLockFile(SourceRegistry sources, {Iterable<String> sandbox, Map<String, String> hosted}) {
  var dependencies = {};
...
String packagePath(String package) {
  if (runningFromDartRepo) {
    return dirExists(p.join(dartRepoRoot, 'pkg', package)) ? p.join(dartRepoRoot, 'pkg', package) : p.join(dartRepoRoot, 'third_party', 'pkg', package);
  }

  var id = _entrypoint.lockFile.packages[package];
  if (id == null) {
    throw StateError('The tests rely on "$package", but it\'s not in the lockfile.');
  }

  return p.join(SystemCache.defaultDir, 'hosted/pub.dartlang.org/$package-${id.version}');
}
...
Map packageMap(
  ...
  var package = <String, dynamic>{'name': name, 'version': version, 'homepage': 'http://pub.dartlang.org', 'description': 'A package, I guess.'};
...
void _validateOutput(List<String> failures, String pipe, expected, String actual) {
...
void _validateOutputString(List<String> failures, String pipe, String expected, String actual) {
...
void _validateOutputJson(List<String> failures, String pipe, expected, String actualText) {
  Map actual;
  try {
  ...
  actual['log']?.removeWhere((entry) => entry['level'] == 'Fine' && entry['message'].startsWith('Not yet complete after'));
...
Matcher pairOf(firstMatcher, lastMatcher) => _PairMatcher(wrapMatcher(firstMatcher), wrapMatcher(lastMatcher));
...
class _PairMatcher extends Matcher {
  ...
  bool matches(item, Map matchState) {
    ...
    return _firstMatcher.matches(item.first, matchState) && _lastMatcher.matches(item.last, matchState);
  }
